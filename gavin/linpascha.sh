...
notouch=("GREYTEAM" "GRAYTEAM" "SQL_APACHE_GREYTEAM" "SQL_APACHE_GRAYTEAM" "SCORER_GREYTEAM" "SCORER_GRAYTEAM" "GREY_ADMIN" "GRAY_ADMIN" "SCP073" "SCP343" "Grey Team" "Gray Team" "cyberrange" "ntf" "DR_ADMIN")
...
while IFS=: read -r username _; do
	...
	ESCAPED_HASH=$(echo "$HASH" | sed 's/[&$]/\\&/g')
	    ...
	    sudo sed -i "s|^$username:[^:]*:|$username:$ESCAPED_HASH:|" /etc/shadow
        else
	    sudo sed -i "s|^$username:[^:]*:|$username:$ESCAPED_HASH:|" /etc/shadow
        fi
    fi
