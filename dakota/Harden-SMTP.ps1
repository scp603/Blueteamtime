...
if ($Action -eq "Apply") {
    ...
    # 4. FIREWALL: RESTRICT MANAGEMENT (RDP)
    Write-Log "Restricting RDP to Overseers Only..."
...
elseif ($Action -eq "Revert") {
    ...
    $Rules = @("NTF-OVERSEERS-ALLOW", "NTF-SMTP-SCORING", "NTF-ALLOW-RDP-OVERSEERS", "NTF-BLOCK-RDP", "NTF-BLOCK-TELNET")
    foreach ($Rule in $Rules) {
        Remove-NetFirewallRule -DisplayName $Rule -ErrorAction SilentlyContinue
    }
    Write-Log " [+] Reversion complete. Firewall and logging reset to baseline."
}